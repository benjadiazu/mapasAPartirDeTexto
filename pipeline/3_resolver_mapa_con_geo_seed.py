# resolver_mapa_con_geo_seed.py
# Igual que 3_resolver_mapa.py pero aplica el seed geográfico a TODOS los nodos
# presentes en seed_layout (no solo pivotes) y con ventana más ajustada.
import json
import math
...
# El seed_layout aquí contiene coordenadas geográficas reales (desde 2_con_geo_seed)
seed_layout = ineq_data.get("seed_layout", {})
...
def solve_hybrid():
    ...
    # ========================================================
    # Layout inicial: geo seed tiene prioridad sobre clusters
    # ========================================================
    ...
    # El seed geográfico real sobreescribe el seed de clusters
    final_seed = dict(hierarchical_seed)
    final_seed.update(seed_layout)

    # Ventana más ajustada que el solver original (SIDE//6 → SIDE//15)
    # para que las coordenadas reales tengan mayor influencia
    SEED_WINDOW = max(60, min(WIDTH, HEIGHT) // 15)
    print(f"[GEO SEED] Ventana de seed: ±{SEED_WINDOW}px sobre {len(seed_layout)} nodos")

    geo_seed_names = set(seed_layout.keys())

    for p, coord in final_seed.items():
        if p not in x:
            continue

        # Aplicar seed a TODOS los nodos georeferenciados (no solo pivotes)
        # Para el resto del hierarchical_seed, solo aplicar si son pivotes
        if p in geo_seed_names or p in pivotes:
    ...
    return {
        ...
        "height": HEIGHT,
        "rel_eval": rel_eval,
        "solver_mode": "geo_seed_constraints"
    }
