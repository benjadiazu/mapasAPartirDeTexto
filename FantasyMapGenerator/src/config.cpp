...
bool parseOptions(int argc, char **argv) {
    ...
    void *argtable[] = {
        ...
        opts.locations    = arg_filen(NULL, "locations", "<file>", 0, 1, "JSON file with city/town positions from solution_refined.json"),
        opts.end          = arg_end(20)
    };
...
bool _setOptions(OptionArgs opts) {
    ...
    if (!_setLocationsFile(opts.locations)) { return false; }
...
bool _setLocationsFile(arg_file *locations) {
    if (locations->count > 0) {
        gen::config::locationsFile = std::string(locations->filename[0]);
    }
