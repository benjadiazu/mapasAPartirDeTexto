...
int main(int argc, char **argv) {
    ...
    if (!gen::config::locationsFile.empty()) {
        // Place cities/towns at positions loaded from external JSON file
        gen::config::print("Loading locations from: " + gen::config::locationsFile);
        std::ifstream locFile(gen::config::locationsFile);
        std::string locJsonStr((std::istreambuf_iterator<char>(locFile)),
                                std::istreambuf_iterator<char>());
        jsoncons::json locJson = jsoncons::json::parse(locJsonStr);

        if (locJson.has_member("cities") && gen::config::enableCities) {
            jsoncons::json cities = locJson["cities"];
            gen::config::print("Placing " + gen::config::toString(cities.size()) + " cities...");
            timer.reset();
            timer.start();
            for (const auto &c : cities.elements()) {
                std::string name = c["name"].as<std::string>();
                std::string territory = c["territory"].as<std::string>();
                double nx = c["nx"].as<double>();
                double ny = c["ny"].as<double>();
                map.addCityAt(name, territory, nx, ny);
            }
            timer.stop();
            gen::config::print("Finished placing cities in " +
                               gen::config::toString(timer.getTime()) + " seconds.\n");
        }

        if (locJson.has_member("towns") && gen::config::enableTowns) {
            jsoncons::json towns = locJson["towns"];
            gen::config::print("Placing " + gen::config::toString(towns.size()) + " towns...");
            timer.reset();
            timer.start();
            for (const auto &t : towns.elements()) {
                std::string name = t["name"].as<std::string>();
                double nx = t["nx"].as<double>();
                double ny = t["ny"].as<double>();
                map.addTownAt(name, nx, ny);
            }
            timer.stop();
            gen::config::print("Finished placing towns in " +
                               gen::config::toString(timer.getTime()) + " seconds.\n");
        }
    } else {
