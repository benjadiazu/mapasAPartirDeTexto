...
int gen::MapGenerator::_getFaceAtPoint(double nx, double ny) {
    double x = _extents.minx + nx * (_extents.maxx - _extents.minx);
    double y = _extents.miny + ny * (_extents.maxy - _extents.miny);

    std::vector<dcel::Point> facePositions = _computeFacePositions();
    double minDist = std::numeric_limits<double>::infinity();
    int bestFace = -1;
    for (unsigned int i = 0; i < facePositions.size(); i++) {
        dcel::Point fp = facePositions[i];
        if (!_extents.containsPoint(fp)) { continue; }
        double dx = fp.x - x;
        double dy = fp.y - y;
...
void gen::MapGenerator::_ensureLandAt(double nx, double ny) {
    double x = _extents.minx + nx * (_extents.maxx - _extents.minx);
    double y = _extents.miny + ny * (_extents.maxy - _extents.miny);
    // Radius in map units: covers ~200 Voronoi faces, safely above the
    // 35-face island cleanup threshold so the patch won't be erased.
    double radius2 = 0.8 * 0.8;
    double minHeight = _isolevel + 0.05;
    ...
    for (unsigned int i = 0; i < _vertexMap.vertices.size(); i++) {
        ...
        if (dx*dx + dy*dy <= radius2) {
            ...
            if (h < minHeight) {
                _heightMap.set(i, minHeight);
            }
        }
    }
    // Invalidate cached land-face table since heights changed.
    _isLandFaceTableInitialized = false;
}

void gen::MapGenerator::addCityAt(std::string cityName, std::string territoryName,
                                  double nx, double ny) {
    if (!_isInitialized) {
        throw std::runtime_error("MapGenerator must be initialized.");
    }
    ...
    _ensureLandAt(nx, ny);
    int faceid = _getFaceAtPoint(nx, ny);

    City city;
    city.cityName = cityName;
    city.territoryName = territoryName;
    city.position = _computeFacePosition(faceid);
    city.faceid = faceid;
    _updateCityMovementCost(city);
    _cities.push_back(city);
}

void gen::MapGenerator::addTownAt(std::string townName, double nx, double ny) {
    if (!_isInitialized) {
        throw std::runtime_error("MapGenerator must be initialized.");
    }
    if (!_isHeightMapEroded) {
    ...
    Town town;
    town.townName = townName;
    town.position = _computeFacePosition(faceid);
    town.faceid = faceid;
    _towns.push_back(town);
}
